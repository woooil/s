    ...
    <>
      <defs>
        <marker id={id} markerWidth="10" markerHeight="8" refX="0" refY="4" orient="auto">
          <path d="M 10 0 L 0 4 10 8 8 4 Z" fill="black"/>
        </marker>
      </defs>
      <path
        {...attr}
...
export default function Label({
  ...
  if (resolved.offsite) {
    ...
    const theta = Math.atan2(attr.y - resolved.coord.y, attr.x - resolved.coord.x)
    if (-Math.PI / 4 < theta && theta <= Math.PI / 4) attr.textAnchor = 'start'
    else if (Math.PI / 4 < theta && theta <= Math.PI * 3 / 4) attr.dominantBaseline = 'text-top'
    else if (Math.PI * 3 / 4 < theta || theta <= -Math.PI * 3 / 4) attr.textAnchor = 'end'
  ...
  return (
    <g {...props}>
      {
        resolved.offsite &&
        <Arrow x1={resolved.coord.x} y1={resolved.coord.y} x2={attr.x} y2={attr.y} />
      }
      <text {...attr}>
        {resolved.label}
      </text>
