...
export function useShapes() {
  ...
  const deleteShape: <T extends Shape>(shape: T, cascade?: boolean) => T = (shape, cascade = true) => {
    setShapes((i) => {
      if (cascade && shape.dependencies.some((i) => shapes.include(i))) {
        throw new Error("Dependencies are still in shapes.")
      }
      return i.filter((j) => 
        !(j === shape || j.dependencies.include(shape)))
    })
    return shape
  }
